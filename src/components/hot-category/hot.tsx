export default function HotCategory(){
    return(
        <><div className="absolute w-[648px] h-[52px] top-[2914px] left-[-52px] font-['Roboto'] text-[34px] font-normal leading-[39.84px] text-center rotate-[-90deg]">EXPLORE NEW AND POPULAR STYLES</div>
        <div className="absolute w-[1320px] h-[648px] top-[2604px] left-[323px]">
            <img src="/item-category 1.png" className="absolute w-[648px] h-[648px]" />
            <div className="absolute left-[1008px]">
                <img src="/item-category-hover 1.png" />
            </div>

            <div className="absolute left-[1008px] top-[336px]">
                <img src="/item-category-hover 1.png" />
            </div>

            <div className="absolute left-[672px] top-[336px]">
                <img src="/item-category-2.png" />
            </div>


            <div className="absolute left-[672px]">
                <img src="/01.png" />
            </div>
        </div></>
    )
}